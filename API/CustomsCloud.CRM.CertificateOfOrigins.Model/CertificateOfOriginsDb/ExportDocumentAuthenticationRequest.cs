...
public partial class ExportDocumentAuthenticationRequest
{
    ...
    // Child collections (loaded via .Include in the by-id read — mirror the legacy LoadProperty calls).
    public List<CustomsItemToExportDocumentAuthenticationRequest> CustomsItems { get; set; } = [];

    public List<ExportDocumentAuthenticationRequestLeadDocument> LeadDocuments { get; set; } = [];

    public List<ExportAuthenticationRequestManufacturingArea> ManufacturingAreas { get; set; } = [];
}
