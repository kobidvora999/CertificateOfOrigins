    ...
    : BaseMicroServiceProxyAdapter(restProxy, CustomsMicroServices.Documents), IDocumentsProxy
{
    public async Task<List<DocumentDto>?> GetDocumentsByEntity(int entityId, int entityTypeId)
    {
        var req = CreateRequestBuilder()
            .UseGetMethod()
            .WithResource("Documents/DocumentsByEntity") // TODO: confirm endpoint name (route = target BL name) with the Documents microservice
    ...
    public async Task<List<DocumentDto>?> GetDocumentsByIds(List<int> documentIds)
    {
        var req = CreateRequestBuilder()
            .UsePostMethod()
            .WithResource("Documents/DocumentsByIds") // TODO: confirm endpoint name (route = target BL name) with the Documents microservice
            .AddBody(documentIds);
        return (await ExecuteAsync<List<DocumentDto>>(req)).Data;
    }
    ...
    public async Task<bool> DeleteDocuments(List<int> documentIds, int entityId, int entityTypeId)
    {
        var req = CreateRequestBuilder()
            .UsePostMethod()
            .WithResource("Documents/DeleteDocuments") // TODO: confirm endpoint name (route = target BL name) with the Documents microservice
            .AddQueryStringParameter("entityId", entityId)
            .AddQueryStringParameter("entityTypeId", entityTypeId)
            .AddBody(documentIds);
    ...
    public async Task<DocumentDto?> UploadDocumentAndSave(DocumentDto document, byte[] content)
    {
        var req = CreateRequestBuilder()
            .UsePostMethod()
            .WithResource("Documents/UploadDocumentAndSave") // TODO: confirm endpoint name + payload shape with the Documents microservice
