...
CREATE OR ALTER PROCEDURE [CRM].[usp_CertificateOfOrigins_GetCertificateOfOriginsByFilter_test]
...
SELECT      convert(int,1) ID, -- not null

      convert([nvarchar](35),'abcdefghijklmnop1234567890') [CertificateNumber], -- not null

      convert([nvarchar](255),'abcdefghijklmnop1234567890') [Name] , -- not null

      convert(int,1) [CustomesAgentID],-- not null

      convert([nvarchar](255),'abcdefghijklmnop1234567890') [CustomesAgentTitle],-- not null

      convert([char](9),'abcdefghi') [CustomesAgentExternalIdNum] ,-- not null

      convert(int,1) [ExporterID],-- not null

      convert([nvarchar](255),'abcdefghijklmnop1234567890') [ExporterTitle],-- not null

      convert([char](9),'abcdefghi') [ExporterExternalIdNum],-- not null

      convert([varchar](1),'a') [ExportDeclarationNum]
...
ORDER BY F.CreateDate DESC
  ...
  /**/      
    ...
    @requestReasonID INT',     @certificateNumber = @certificateNumber ,
